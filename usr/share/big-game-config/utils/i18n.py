...
if 'APPIMAGE' in os.environ or 'APPDIR' in os.environ:
    ...
    # translation_utils.py is in: usr/share/big-game-config/utils/translation_utils.py
    # We need to get to: usr/share/locale
    script_dir = os.path.dirname(os.path.abspath(__file__))  # usr/share/big-game-config/utils
    app_dir = os.path.dirname(script_dir)                    # usr/share/big-game-config
...
# Configure the translation text domain for big-game-config
gettext.bindtextdomain("big-game-config", locale_dir)
gettext.textdomain("big-game-config")
